...
class MEMO(BaseLearner):
        ...
        if self._cur_task == 0:
            if self.args['train_base']:
                ...
                for param in self._network.TaskAgnosticPartLearner.parameters():
                    param.requires_grad = True
                #self._network.part_prototypes.requires_grad = False
            else:
        ...
        if self._cur_task > 0:
            for i in range(self._cur_task):
                if self.args['train_adaptive']:
                    ...
                    #for p in self._network.AdaptivePartLearners[i].parameters():
                    #    p.requires_grad = True
                    for p in self._network.AdaptiveLearnerSelectors[i].parameters():
                        p.requires_grad = True
            ...
            for i in range(self._cur_task - 1):
                if self.args['train_adaptive']:
                    for j in range(i + 1):
                        ...
                        #self._network.biases[i][j].alpha.weight.requires_grad = True
                        #self._network.biases[i][j].alpha.bias.requires_grad = True
                else:
                    for j in range(i + 1):
                        self._network.biases[i][j].alpha.requires_grad = False
                        #self._network.biases[i][j].alpha.weight.requires_grad = False
                        #self._network.biases[i][j].alpha.bias.requires_grad = False
        ...
        if self._cur_task >= 1:
            for i in range(self._cur_task):
                if self.args['train_adaptive']:
                    ...
                    #self._network.AdaptivePartLearners[i].train()
                    self._network.AdaptiveLearnerSelectors[i].train()
                else:
                    self._network.AdaptiveExtractors[i].eval()
                    #self._network.AdaptivePartLearners[i].eval()
                    self._network.AdaptiveLearnerSelectors[i].eval()
        if len(self._multiple_gpus) > 1:
            self._network = nn.DataParallel(self._network, self._multiple_gpus)
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler, criterion):
        ...
        best_test_acc = 0.
        #best_codebook = np.zeros((50 * 5, 256))
        #losses_ce = np.zeros(self.args['init_epoch'])
        #losses_cpd = np.zeros(self.args['init_epoch'])
        #losses_sd = np.zeros(self.args['init_epoch'])
        #losses_sc = np.zeros(self.args['init_epoch'])
        #losses_cd = np.zeros(self.args['init_epoch'])
        #losses_recon = np.zeros(self.args['init_epoch'])
        #losses_latent = np.zeros(self.args['init_epoch'])
        for _, epoch in enumerate(prog_bar):
            self._network.train()
            losses = 0.
            correct, total = 0, 0
            for i, (_, points, points_bar, targets) in enumerate(train_loader):
                #points, targets = points.to(self._device), targets.to(self._device)
                ...
                #trans_feat = outputs['trans_feat']

                #loss = F.nll_loss(F.log_softmax(logits, dim=-1), targets) + 0.001 * feature_transform_reguliarzer(trans_feat)
                logits, loss_clf, loss_batch_list = criterion(outputs, targets, outputs_bar, 0)
                ##aux_out = self._network.AdaptiveLearnerSelectors[self._cur_task](outputs['backbone_feats'])
                ##loss_aux = self._network.AdaptiveLearnerSelectors[self._cur_task].loss_function(outputs['backbone_feats'], aux_out)
                ...
                optimizer.step()
                losses += loss.item()
                #losses_ce[epoch] += loss_batch_list['loss_ce'] / len(train_loader)
                #losses_cpd[epoch] += loss_batch_list['loss_cpd'] / len(train_loader)
                #losses_sd[epoch] += loss_batch_list['loss_sd'] / len(train_loader)
                #losses_sc[epoch] += loss_batch_list['loss_sc'] / len(train_loader)
                #losses_cd[epoch] += loss_batch_list['loss_cd'] / len(train_loader)
                #losses_recon[epoch] += loss_batch_list['loss_recon'] / len(train_loader)
                #losses_latent[epoch] += loss_batch_list['loss_latent'] / len(train_loader)
            ...
            if epoch % 5 == 0:
                ...
                if test_acc > best_test_acc:
                    ...
                    best_class_test_acc = class_test_acc
                    #np.save('./vis/25_train/feat_{}.npy'.format(self._cur_task), feat.cpu().numpy())
                    #np.save('./vis/25_train/part_related_{}.npy'.format(self._cur_task), part_related.cpu().numpy())
                    #np.save('./vis/25_train/attn_{}.npy'.format(self._cur_task), attn.cpu().numpy())
                    #np.save('./log/classification/PARTonly_v18/best_acc_{}.npy'.format(self._cur_task), best_class_test_acc)
                    #best_codebook = self._network.part_prototypes.detach().cpu().numpy()
                    #np.save('./log/classification/PARTonly_v1/best_codebook.npy', best_codebook)
                    #_checkpoint_cpu = copy.deepcopy(self._network)
                    #_checkpoint_cpu.cpu()
                    #save_dict = {
                    #    "tasks": self._cur_task,
                    #    "network": _checkpoint_cpu.state_dict(),
                    #    "test_acc": best_test_acc
                    #}
                    #torch.save(save_dict, './log/classification/PARTonly_v18/best_codebook_{}.pkl'.format(self._cur_task))
            else:
        ...
        logging.info(info)
        #plt.plot(np.arange(self.args['init_epoch']), losses_ce)
        #plt.plot(np.arange(self.args['init_epoch']), losses_cpd)
        #plt.plot(np.arange(self.args['init_epoch']), losses_sd)
        #plt.plot(np.arange(self.args['init_epoch']), losses_sc)
        #plt.plot(np.arange(self.args['init_epoch']), losses_cd)
        #plt.plot(np.arange(self.args['init_epoch']), losses_recon)
        #plt.plot(np.arange(self.args['init_epoch']), losses_latent)
        #plt.legend(labels=['losses_ce', 'losses_sc', 'losses_cd'])#, 'losses_recon', 'losses_latent'])#, 'losses_cd'])
        #plt.savefig('./log/classification/PARTonly_co3d_v1/Task 0.png')
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler, criterion):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, points, points_bar, targets) in enumerate(train_loader):
                ...
                outputs_bar = self._network(points_bar)
                #with torch.no_grad():
                #    teacher_outputs = self._teacher_network(points)
                #    teacher_outputs_bar = self._teacher_network(points_bar)
                #outputs.update({'logits_old': teacher_outputs["logits"]})
                #outputs_bar.update({'logits_old': teacher_outputs_bar["logits"]})

                #logits, aux_logits, trans_feat = outputs["logits"], outputs["aux_logits"], outputs['trans_feat']
                #loss_clf = F.nll_loss(F.log_softmax(logits, dim=-1), targets) + 0.001 * feature_transform_reguliarzer(trans_feat)
                #aux_targets = targets.clone()
                #aux_targets = torch.where(aux_targets - self._known_classes+1 > 0,  aux_targets-self._known_classes + 1, 0)
                #loss_aux = F.nll_loss(F.log_softmax(aux_logits, dim=-1), aux_targets)
                ...
                #aux_targets = torch.where(aux_targets - self._known_classes + 1 > 0,  aux_targets-self._known_classes + 1, 0)
                #loss_aux = F.nll_loss(F.log_softmax(aux_logits, dim=-1), aux_targets)
                loss = loss_clf# + loss_aux# + self.args['alpha_aux'] * loss_aux
